...
class RegistroNotas
{
    ...
    public function addCabecalhoDisciplina($disciplina)//cabeçalho turma
    {
        ...
        $this->pdf->SetX(20);
        $this->pdf->Cell(800, 12, " Faculdade: ".utf8_decode ($disciplina->faculdade).'                                 Categoria: '.utf8_decode ($disciplina->curso).'                                   C.H.: 90hs', 'LR', 0, 'L');
    ...
    public function addCabecalhoNotas()
    {
      //  $this->pdf->SetY(185);
        ...
        $this->pdf->Cell(370, 14, 'NOME',  1, 0, 'C', 0);
        ...
        $this->pdf->Cell(40,  14, 'MAO', 1, 0, 'C', 0);
        $this->pdf->Cell(40,  14, 'RD', 1, 0, 'C', 0);
        $this->pdf->Cell(40,  14, 'RT', 1, 0, 'C', 0);
       //$this->pdf->Cell(40,  14, 'AF', 1, 0, 'C', 0);
        $this->pdf->Cell(40,  14, 'AS', 1, 0, 'C', 0);
        $this->pdf->Cell(40,  14, 'EF', 1, 0, 'C', 0);
...
if($students->roleid ==='15'){
    $estudantedp = "- (DP)";
       ...
       // $this->pdf->SetFillColor(200,220,255);
        ...
        if(strlen($nomedisciplina[1])>0){
            $this->pdf->Cell(370, 14, $res->nome.' '.$estudantedp , 'LB', 0, 'R');
         }else{ $this->pdf->Cell(370, 14, ' '.$res->nome.' '.$estudantedp , 'LB', 0, 'R'); }
        ...
        if(strlen($res->RD)>0){
            $this->pdf->Cell(40,  14, number_format($res->RD, 1), 'LB', 0, 'C');
        } else $this->pdf->Cell(40,  14, '-', 'LB', 0, 'C');


         if(strlen($res->RT)>0){
            $this->pdf->Cell(40,  14, number_format($res->RT, 1), 'LB', 0, 'C');
        } else $this->pdf->Cell(40,  14, '-', 'LB', 0, 'C');

    //    $this->pdf->Cell(70,  14, '', 'LB', 0, 'C');

        if(strlen($res->AS)>0){
            $this->pdf->Cell(40,  14, number_format($res->AS, 1), 'LB', 0, 'C');
        } else $this->pdf->Cell(40,  14, '-', 'LB', 0, 'C');
        
        if(strlen($res->EF)>0){
            $this->pdf->Cell(40,  14, number_format($res->EF, 1), 'LBR', 0, 'C');
        } else $this->pdf->Cell(40,  14, '-', 'LBR', 0, 'C');
    ...
    public function addRodapeNota()
    {
        ...
        $this->pdf->Ln(20);
        $this->pdf->SetX(20);
        $this->pdf->Cell(800,  10, 'OBS: MAO - Média das Avaliações Online, RD - Rendimento por Disciplina, RT - Rendimento Total, AS - Avaliação Substitutiva, EF - Exame Final  ', '', 0, 'L');
        
      /*  $this->pdf->Ln(12);
        $this->pdf->SetX(20);
        $this->pdf->Cell(800,  10, ']', '', 0, 'L');
        */
        $this->pdf->Ln(15);
        $this->pdf->SetX(20);
        ...
        $this->pdf->Ln(5);
        $this->pdf->SetX(20);
        $this->pdf->Cell(800,  12, 'Relatório para Conferência de notas Moodle EaD UFGD, exportado por: '.utf8_decode($USER->firstname).' '.utf8_decode($USER->lastname).', '.$data[mday].'/'.$data[mon].'/'.$data[year].'  -  '.number_format($data[hours],0).':'.number_format($data[minutes],0).':'.number_format($data[seconds],0).'  -  '.$CFG->wwwroot, '', 0, 'C');
    }

   //number_format($data[seconds],0)
    /**
