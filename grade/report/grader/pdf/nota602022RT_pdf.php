...
$context = context_course::instance($course->id);
  ...
  $error=null;
    ...
    include_once 'relatorioNotas602022.class.php';
  ...
  //  $arrayRT = get_notas($coursePfId, get_itensid($coursePfId, '', 'category'));
   // $arrayRT = get_notas($course->id, get_itensid($course->id, 'AF', 'mod')); ///RT
    $arrayRD = get_notas($coursePfId , get_itensid($coursePfId , $course->id, 'mod'));//RD



//echo '<pre>';
  //  print_r($arrayRD);
    //$arrayRT = get_notas($coursePfId , get_itensid($coursePfId , 'A4', 'mod'));
   ...
   // $arrayA6 = get_notas($course->id, get_itensid($course->id, 'A6', 'mod'));
    ...
    foreach ($alunos as $aluno) {
      ...
      //  $listAlunoNotas->RT = $arrayRT[$aluno->id]->finalgrade;
        ...
        $listAlunoNotas->RT = estudantes($course->id,$aluno->id);

      //  print_r($listAlunoNotas->RT);
...
function estudantes($courseidOrigem, $aluno) {
  GLOBAL $DB;
...
// verificar se tem idnumber
function get_notas_A($courseid, $itemid, $userid){
    global $DB, $error;
    //"SELECT u.id, u.username, u.idnumber, g.itemid,i.itemname, i.itemtype,i.courseid,g.finalgrade //Debug
    $sql = "SELECT u.id, g.itemid,g.finalgrade
              FROM {grade_grades} g 
        INNER JOIN {grade_items} i ON g.itemid=i.id INNER JOIN {user} u ON u.id=g.userid INNER JOIN {user_enrolments} ue ON ue.userid=g.userid 
             WHERE i.courseid= ? AND i.id=? AND ue.status = '0' AND ue.userid=? ORDER BY g.userid ASC";

    return  $DB->get_record_sql($sql,array($courseid, $itemid, $userid));  
}
