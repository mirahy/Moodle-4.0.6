...
$context = context_course::instance($course->id);
  ...
  $error=null;
    ...
    include_once 'relatorioNotas60.class.php';
    ...
    $arrayPfL = get_notas($course->id, get_itensid($course->id, 'AFL', 'mod'));
    $arrayPsL = get_notas($course->id, get_itensid($course->id, 'ASL', 'mod'));
    $arrayEfL = get_notas($course->id, get_itensid($course->id, 'EFL', 'mod'));

    $arrayA1L = get_notas($course->id, get_itensid($course->id, 'A1L', 'mod'));
    $arrayA2L = get_notas($course->id, get_itensid($course->id, 'A2L', 'mod'));
    $arrayA3L = get_notas($course->id, get_itensid($course->id, 'A3L', 'mod'));
    $arrayA4L = get_notas($course->id, get_itensid($course->id, 'A4L', 'mod'));
  ...
  //  $arrayA5 = get_notas($course->id, get_itensid($course->id, 'A5', 'mod'));
   // $arrayA6 = get_notas($course->id, get_itensid($course->id, 'A6', 'mod'));
    ...
    $alunos1 = array_diff_key($alunosm, $alunoss);

    $alunosmL = get_matriculadosL($course->id, 5);
    $alunos = array_diff_key($alunos1, $alunosmL);
...
//        $listAlunoNotas->A5 = $arrayA5[$aluno->id]->finalgrade;
  //      $listAlunoNotas->A6 = $arrayA6[$aluno->id]->finalgrade;
    ...
    foreach ($alunosmL as $aluno) {
        ...
        $listAlunoNotas->AF = $arrayPfL[$aluno->id]->finalgrade;
...
/*
      //  if ((number_format($somaNotas, 2) != number_format($listAlunoNotas->AO,2)) AND $c == 0){ //round(1.95583, 2);
        if ((round($somaNotas, 1) != round($listAlunoNotas->AO,1)) AND $c == 0){ //round(1.95583, 2);
             $error .= '<div class="alert alert-danger" role="alert">'.round($somaNotas, 2).' A '.round($listAlunoNotas->AO,2).'média AO está divergente do padrão de 4 atividades('.$c.'), confira o relatório de notas</b>;</div>';
             $c++;
         }
    */
        $listAlunoNotas->count = $count;
    ...
    if(is_null($gradeitemid->id) AND $idnumber!='AO' ){
       ...
       // $error .= '<a href="https://docs.google.com/document/d/1Iwb0w-loDWqqgCqUwTvOkI6fC8t6shcsKn5qTws-eLw/edit?usp=sharing">Manual</a>';
        ...
        if($gradeitemid->grademax != 10.0 AND $idnumber!='AO'){
         ...
         if($gradeitemid->fullname != 'AO' AND $idnumber!='AF' AND $idnumber!='AS' AND $idnumber!='EF' AND $idnumber!='AO' AND $idnumber!='A1L' AND $idnumber!='A2L' AND $idnumber!='A3L' AND $idnumber!='A4L'AND $idnumber!='AFL' AND $idnumber!='ASL' AND $idnumber!='EFL'){
...
function get_matriculadosL($courseid, $roleid){
    global $DB;

     $sql = "SELECT u.id, u.username,u.idnumber, u.firstname, u.lastname, rs.roleid
               FROM {role_assignments} rs 
         INNER JOIN {user} u ON u.id=rs.userid INNER JOIN {context} c ON rs.contextid=c.id
              WHERE c.contextlevel=50 AND c.instanceid=? AND rs.roleid=? AND u.username = '02132186154' ORDER BY u.firstname ASC";
