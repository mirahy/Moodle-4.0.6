...
$coursePfId      = required_param('pf', PARAM_INT); 
$coursePfIdl      = required_param('pfl', PARAM_INT);
...
$context = context_course::instance($course->id);
  ...
  $error=null;
    ...
    include_once 'relatorioNotas602022L.class.php';
    ...
    $disciplina->professor = mb_strtoupper($professores->formadores);
    $disciplina->tutor = mb_strtoupper($professores->tutores);
    ...
    $arrayAo = get_notas($course->id, get_itensid($course->id, '', 'category','AO'));
    
    $arrayPs = get_notas($course->id, get_itensid($course->id, 'AS', 'mod',null));
    $arrayEf = get_notas($course->id, get_itensid($course->id, 'EF', 'mod',null));

    $arrayA1 = get_notas($course->id, get_itensid($course->id, 'A1', 'mod',null));
    $arrayA2 = get_notas($course->id, get_itensid($course->id, 'A2', 'mod',null));
    $arrayA3 = get_notas($course->id, get_itensid($course->id, 'A3', 'mod',null));
    $arrayA4 = get_notas($course->id, get_itensid($course->id, 'A4', 'mod',null));

    $arrayRT = get_notas($coursePfId, get_itensid($coursePfId, '', 'category','AO'));
   // $arrayRT = get_notas($course->id, get_itensid($course->id, 'AF', 'mod')); ///RT
    $arrayRD = get_notas($coursePfId , get_itensid($coursePfId , $course->id, 'mod',null));//RD

##LIBRAS
    $arrayAoL = get_notas($course->id, get_itensid($course->id, '', 'category','AOL'));

    $arrayPsL = get_notas($course->id, get_itensid($course->id, 'ASL', 'mod',null));
    $arrayEfL = get_notas($course->id, get_itensid($course->id, 'EFL', 'mod',null));

    $arrayA1L = get_notas($course->id, get_itensid($course->id, 'A1L', 'mod',null));
    $arrayA2L = get_notas($course->id, get_itensid($course->id, 'A2L', 'mod',null));
    $arrayA3L = get_notas($course->id, get_itensid($course->id, 'A3L', 'mod',null));
    $arrayA4L = get_notas($course->id, get_itensid($course->id, 'A4L', 'mod',null));

    $arrayRTL = get_notas($coursePfIdl, get_itensid($coursePfIdl, '', 'category','AOL'));
   // $arrayRT = get_notas($course->id, get_itensid($course->id, 'AF', 'mod')); ///RT
    ...
    $alunosml = get_matriculados($coursePfIdl, 5); //Aluno UAB matriculado no provao LIBRAS

   // $alunoss = get_alunos_suspensos($course->id);
    $alunos = array_diff_key($alunosm, $alunosml); //Retira o aluno matriculado no provao LIBRAS
    ...
    foreach ($alunos as $aluno) {
        ...
        $listAlunoNotas->nome = utf8_decode (mb_strtoupper($aluno->firstname)).' '.utf8_decode (mb_strtoupper($aluno->lastname));
        $listAlunoNotas->AO = $arrayAo[$aluno->id]->finalgrade;

        $listAlunoNotas->RD = $arrayRD[$aluno->id]->finalgrade;
        $listAlunoNotas->RT = $arrayRT[$aluno->id]->finalgrade;
    ...
    //    print_r ($listAlunoNotas);

//        $listAlunoNotas->A5 = $arrayA5[$aluno->id]->finalgrade;
  //      $listAlunoNotas->A6 = $arrayA6[$aluno->id]->finalgrade;

       // $somaNotas = ($listAlunoNotas->A1 + $listAlunoNotas->A2 + $listAlunoNotas->A3 + $listAlunoNotas->A4) /4;
        $somaNotas = ($listAlunoNotas->A1 + ($listAlunoNotas->A2*2) + $listAlunoNotas->A3 + ($listAlunoNotas->A4*2)) /6;
//$listAlunoNotas->AO = $somaNotas;
      //  if ((number_format($somaNotas, 2) != number_format($listAlunoNotas->AO,2)) AND $c == 0){ //round(1.95583, 2);
        if ((round($somaNotas, 1) != round($listAlunoNotas->AO,1)) AND $c == 0){ //round(1.95583, 2);
             $error .= '<div class="alert alert-danger" role="alert">'.round($somaNotas, 1).' A '.round($listAlunoNotas->AO,1).'média AO está divergente do padrão de 4 atividades('.$c.'), confira o relatório de notas</b>;</div>';
             $c++;
         }
    ...
    foreach ($alunosml as $aluno) {
        ...
        $listAlunoNotas->nome = utf8_decode (mb_strtoupper($aluno->firstname)).' '.utf8_decode (mb_strtoupper($aluno->lastname));
        $listAlunoNotas->AO = $arrayAoL[$aluno->id]->finalgrade;

        $listAlunoNotas->RD = $arrayRDL[$aluno->id]->finalgrade;
        $listAlunoNotas->RT = $arrayRTL[$aluno->id]->finalgrade;

        $listAlunoNotas->AS = $arrayPsL[$aluno->id]->finalgrade;
        $listAlunoNotas->EF = $arrayEfL[$aluno->id]->finalgrade;

        $listAlunoNotas->A1 = $arrayA1L[$aluno->id]->finalgrade;
        $listAlunoNotas->A2 = $arrayA2L[$aluno->id]->finalgrade;
        $listAlunoNotas->A3 = $arrayA3L[$aluno->id]->finalgrade;
        $listAlunoNotas->A4 = $arrayA4L[$aluno->id]->finalgrade;
...
/*    echo '<br>Leia o tutorial de como ajustar os parametros para exibir o Relatório de Notas:<br><br>';
    echo '<iframe width="100%" height="600" src="https://docs.google.com/document/d/e/2PACX-1vS7fUUzGJmvsUjRX0bb8Z5vww9LBco4AnmMQO1EmsUFftQJE734EGj8FFb24n72ZwMTOclWhZhQXyrE/pub?embedded=true"></iframe>';
   // echo '<iframe width="100%"" height="600" src="https://docs.google.com/document/d/1Iwb0w-loDWqqgCqUwTvOkI6fC8t6shcsKn5qTws-eLw/edit"></iframe>';
  */  
#} else{
...
function get_itensid($courseid, $idnumber, $itemtype, $categoryname){
    global $DB, $error;

    if(!is_null($categoryname)){
         $sql = "SELECT i.id,i.itemname, i.idnumber, i.iteminstance, i.itemtype,i.courseid,i.grademax, gc.fullname
              FROM  {grade_items} i JOIN  {grade_categories} gc ON i.iteminstance = gc.id
             WHERE i.courseid= ? AND i.idnumber = ? AND i.itemtype = ? AND i.gradetype = '1' AND (gc.fullname = ?)";
        // WHERE i.courseid= ? AND i.idnumber =? AND i.itemtype = ? AND i.gradetype = '1' AND i.grademax = '10.000' ";
    $gradeitemid = $DB->get_record_sql($sql,array($courseid, $idnumber, $itemtype,$categoryname));
...
/*
    ...
    if(is_null($gradeitemid->id) AND $idnumber!='AO' ){
       ...
       // $error .= '<a href="https://docs.google.com/document/d/1Iwb0w-loDWqqgCqUwTvOkI6fC8t6shcsKn5qTws-eLw/edit?usp=sharing">Manual</a>';
    }else{
