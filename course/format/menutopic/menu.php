        ...
        if (isset($menunode->subtopics) && is_array($menunode->subtopics) && count($menunode->subtopics) > 0) {
            ...
            if (!empty($menunode->target)) {
             ...
             $menuname =  $menunode->name;
                if ($menuname ==="Sala de Aula Virtual") {
                    $name = '<i class="fa fa-pencil-square-o fa-lg" aria-hidden="true"></i>';
                    $link_properties['class'] .= '   alert-sala';
                }
                 if ($menuname ==="Conteúdo Interativo") {
                    $name = '<i class="fa fa-pencil-square-o fa-lg" aria-hidden="true"></i>';
                    $link_properties['class'] .= '   alert-sala';
                }

                if ($menuname ==="Conteúdo do curso") {
                    $name = '<i class="fa fa-pencil-square-o fa-lg" aria-hidden="true"></i>';
                    $link_properties['class'] .= '   alert-sala';
                }

                if ($menuname ==="Avaliação") {
                    $name = '<i class="fa fa-pencil-square-o fa-lg" aria-hidden="true"></i>';
                    $link_properties['class'] .= '   alert-subs';
                }

            $content .= html_writer::link($url, $name.' <b>'.$menunode->name.'</b>' , $link_properties);
           // $content .= html_writer::link($url, $menunode->name, $linkproperties);
            ...
            if (isset($menunode->hidden) && $menunode->hidden) {
             ...
             $menuname =  $menunode->name;
            $name = '';
          if ($menuname ==="Apresentação") {
                 $name = '<i class="fa fa-id-card-o  fa-lg" aria-hidden="true"></i>';
               //  $li_properties['class'] .= ' alert alert-success';
                 $link_properties['class'] .= '   alert-apresentacao';
             }
              if ($menuname ==="Biblioteca") {
                 $name = '<i class="fa fa-book  fa-lg" aria-hidden="true"></i>';
               //  $li_properties['class'] .= ' alert alert-success';
                 $link_properties['class'] .= '   alert-biblioteca';
             }
                if ($menuname ==="Espaço Informativo") {
                 $name = '<i class="fa fa-comments fa-lg" aria-hidden="true"></i>';
              //   $li_properties['class'] .= ' alert alert-warning';
                 $link_properties['class'] .= '   alert-interacao';
             }
              if ($menuname ==="Espaço de Interação") {
                 $name = '<i class="fa fa-comments fa-lg" aria-hidden="true"></i>';
              //   $li_properties['class'] .= ' alert alert-warning';
                 $link_properties['class'] .= '   alert-interacao';
             }
             if ($menuname ==="Avaliações Online") {
                 $name = '<i class="fa fa-pencil-square-o fa-lg" aria-hidden="true"></i>';
               //  $li_properties['class'] .= ' alert alert-dange';
                 $link_properties['class'] .= '  alert-subs';
             }
              ...
              if ($menuname ==="Provão") {
               ...
               if ($menuname ==="Avalie a Disciplina") {
                 $name = '<i class="fa fa-pencil-square-o fa-lg" aria-hidden="true"></i>';
               //  $li_properties['class'] .= ' alert alert-dange';
                 $link_properties['class'] .= '  alert-avalie';
             }

            if ($menuname ==="Acompanhe suas notas") {
                 $name = '<i class="fa fa-list-alt fa-lg" aria-hidden="true"></i> ';
               //  $li_properties['class'] .= ' alert alert-dange';
                 $link_properties['class'] .= '  alert-notas';
             }
              if ($menuname ==="Avaliação Final") {
                 $name = '<i class="fa fa-pencil-square-o fa-lg" aria-hidden="true"></i> ';
               //  $li_properties['class'] .= ' alert alert-dange';
                 $link_properties['class'] .= '  alert-infog';
             }

               if ($menuname ==="Aula 1") {
                 $name = '<i class="fa fa-pencil-square-o fa-lg" aria-hidden="true"></i>';
                // $li_properties['class'] .= ' disabled';
                 $link_properties['class'] .= '  alert-info';
             }
             if ($menuname ==="Aula 2") {
                 $name = '<i class="fa fa-pencil-square-o fa-lg" aria-hidden="true"></i>';
                // $li_properties['class'] .= ' disabled';
